...
function getNetwork() {
  ...
  fetch(`/api/network/${networkId}`)
    ...
    .catch(err => {
      console.log(err);
      alert('Cannot find a network with this id! Taking you back.');
      window.history.back();
    });
}

function printNetwork(networkData) {
  console.log(networkData);

  networkId = networkData._id;

  const { networkName, createdBy, createdAt, comments } = networkData;
...
function handleNewReplySubmit(event) {
  ...
  fetch(`/api/comments/${networkId}/${commentId}`, {
    method: 'PUT',
    headers: {
      Accept: 'application/json',
      'Content-Type': 'application/json'
